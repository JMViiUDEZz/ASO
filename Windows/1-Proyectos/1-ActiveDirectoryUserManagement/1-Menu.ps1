...
while($OPCION -ne 4)
{
    ...
    switch($OPCION)
    {
        1{Invoke-Command -ComputerName w-server -Credential ASIR\Administrador -FilePath C:\ASO\Windows\1-Proyectos\1-ActiveDirectoryUserManagement\1.1-MenuOrganizationalUnits.ps1}
        2{Invoke-Command -ComputerName w-server -Credential ASIR\Administrador -FilePath C:\ASO\Windows\1-Proyectos\1-ActiveDirectoryUserManagement\1.2-MenuGroups.ps1}
        3{Invoke-Command -ComputerName w-server -Credential ASIR\Administrador -FilePath C:\ASO\Windows\1-Proyectos\1-ActiveDirectoryUserManagement\1.3-MenuUsers.ps1}
        4{exit}
    }
